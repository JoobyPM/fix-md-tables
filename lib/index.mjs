...
export function compensateRegularCell(cell, compensation) {
  ...
  // If not enough trailing space for any compensation (need at least 2 for one ideographic), skip
  if (trailingSpace.length < 2) {
    return cell;
  }
  ...
  if (trailingSpace.length >= spacesToRemove) {
    ...
    // Not enough for proportional removal - remove all trailing, add full compensation
...
export function processCell(cell, col, isSeparatorRow, maxEmojiPerCol) {
  ...
  // No compensation needed if no emoji in column at all
  if (maxEmoji === 0) {
    return cell;
  }

  // Formula: base + (max - cell), where base = min(2, max - 1)
  // Even max-emoji cells get the base compensation for visual consistency
...
export function processTable(tableRows) {
  ...
  return originalParsedRows.map((originalRow, rowIdx) => {
    ...
    const processedCells = originalRow.map((originalCell, col) => {
      ...
      // Formula: base + (max - cell), where base = min(2, max - 1)
      const base = Math.min(2, maxEmoji - 1);
      let neededCompensation = base + maxEmoji - cellEmoji;

      // For 0-emoji cells with max > 2: cap at max - 1
      if (cellEmoji === 0 && maxEmoji > 2) {
        neededCompensation = Math.min(neededCompensation, maxEmoji - 1);
      }
      ...
      return isSeparatorRow ? compensateSeparatorCell(cleanedCell, neededCompensation) : compensateRegularCell(cleanedCell, neededCompensation);
    });
