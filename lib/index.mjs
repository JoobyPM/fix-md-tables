...
export function calculateMaxEmojiPerColumn(parsedRows, numCols) {
  const maxEmojiPerCol = Array(numCols).fill(0);
...
export function compensateSeparatorCell(cell, compensation) {
  ...
  const newDashes = dashes.length > dashesToRemove
    ? dashes.slice(0, -dashesToRemove)
    : dashes;
...
export function splitCellContent(cell) {
  ...
  return [
    cell.slice(0, leadingEnd),
    cell.slice(leadingEnd, trailingStart),
    cell.slice(trailingStart),
  ];
}
...
export function processCell(cell, col, isSeparatorRow, maxEmojiPerCol) {
  ...
  return isSeparatorRow
    ? compensateSeparatorCell(cell, compensation)
    : compensateRegularCell(cell, compensation);
}
...
export function processTable(tableRows) {
  ...
  return parsedRows.map((row, rowIdx) => {
    ...
    const processedCells = row.map((cell, col) =>
      processCell(cell, col, isSeparatorRow, maxEmojiPerCol)
    );
